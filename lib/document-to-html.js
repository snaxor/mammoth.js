...
function DocumentConversion(options, comments) {
    ...
    function convertElements(elements, messages, options) {
        return flatMap(elements, function(element) {
            var html = elementToHtml(element, messages, options);
            if (html){
                for (var i = 0; i < html.length; i++){
                    var item = html[i];
                    if (item.tag && element.attributes){
                        item.tag = _.clone(item.tag);
                        item.tag.attributes = _.clone(item.tag.attributes || {});
                        _.defaults(item.tag.attributes, element.attributes);
                        delete item.tag.attributes.classes;
                        if (element.attributes.classes){
                            var classes = item.tag.attributes.class ? [item.tag.attributes.class] : [];
                            classes = classes.concat(element.attributes.classes);
                            item.tag.attributes.class = classes.join(' ');
                        }
                    }
                }
            }
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.isHidden) {
            paths.push(findHtmlPathForRunProperty("hidden"));
        }
        ...
        if (run.highlight) {
            paths.push(findHtmlPathForRunProperty("highlight", "mark"));
        }
