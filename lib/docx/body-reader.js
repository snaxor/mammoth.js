...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            ...
            return {
                ...
                isHidden: readBooleanElement(element.first("w:vanish"))
            };
        });
    }
    ...
    function readFldChar(element) {
        ...
        if (type === "begin") {
            ...
            var complexField = unknownComplexField;
            var hyperlinkOptions = parseHyperlinkFieldCode(currentInstrText.join(''));
            var anchor = parseHyperlinkFieldCodeAnchor(currentInstrText.join(''));
            if (hyperlinkOptions !== null){
                complexField = {type: "hyperlink", options: hyperlinkOptions, anchor: null};
            } else if (anchor !== null){
                complexField = {type: "hyperlink", anchor: anchor, href: null};
            }
            complexFieldStack.pop();
    ...
    function currentHyperlinkAnchor() {
        var topHyperlink = _.last(complexFieldStack.filter(function(complexField) {
            return complexField.type === "hyperlink";
        }));
        return topHyperlink ? topHyperlink.anchor : null;
    }
    ...
    function parseHyperlinkFieldCodeAnchor(code) {
        var result = /\s*HYPERLINK\s+\\l\s*"(.*)"/.exec(code);
        if (result) {
            return result[1];
        } else {
            return null;
    ...
    var xmlElementReaders = {
        ...
        "w:r": function(element) {
            return readXmlElements(element.children)
                .map(function(children) {
                    ...
                    var hyperlinkAnchor = currentHyperlinkAnchor();
                    if (hyperlinkOptions !== null) {
                        children = [new documents.Hyperlink(children, hyperlinkOptions)];
                    } else if (hyperlinkAnchor !== null){
                        children = [new documents.Hyperlink(children, {anchor: hyperlinkAnchor})];
                    }
        ...
        "w:lastRenderedPageBreak": function(element) {
            return elementResult(documents.renderedPageBreak);
        },
...
function readNumberingProperties(styleId, element, numbering) {
    ...
    if (level === undefined || numId === undefined) {
        ...
        if (levelInfo){
            levelInfo = _.clone(levelInfo);
            levelInfo.numId = numId;
        }
        return levelInfo;
    }
}
